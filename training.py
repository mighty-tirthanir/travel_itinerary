...
def extract_summary_spacy(text, num_sentences=8):
    ...
    sentence_scores = {sent: sum(word_frequencies.get(token.text.lower(), 0) for token in sent) for sent in doc.sents}
...
for train_size in train_sizes:
    X_train, X_test, y_train, y_test = train_test_split(X_resampled, y_resampled, train_size=train_size, random_state=42)
